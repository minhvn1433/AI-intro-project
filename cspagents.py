...
class MinesweeperSolve:
    ...
    def uncover_square(self,square):
        ...
        if current.original_constant == 9:
            ...
            if current.original_constant == 0: #KEEP UNCOVER
            ...
            if len(comb) > squares_left:
                ...
                for choice in [0,1]:
                    ...
                    if sum(comb) == mines_left and len(comb) == squares_left:
                        ...
                        if valid: #only take valid solution
                            c = comb.copy()
                            solutions.append(c)
                    backtracker(comb)
                    removed = comb.pop()
                return solutions
        solutions = [] #initiate backtrack to find solution if mines less than squares
        if mines_left < squares_left:
            backtracker([])
        if solutions: #check if any squares are safe in all solutions
    ...
    def get_neighbor_count(self,variable): #return number of neighbor mines, safe and covered squares
        nx,ny = variable
        #get its neighbors
